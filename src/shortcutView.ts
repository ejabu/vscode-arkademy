...
function aNodeWithIdTreeDataProvider(): vscode.TreeDataProvider<{ key: string }> {
	return {
		getChildren: (element: { key: string }): { key: string }[] => {
			...
			return _getChildren(element ? element.key : undefined).map(key => getNode(key));
		},
...
function _getChildren(key: string): string[] {
	console.log("GET CHILDREN FUNCT")
	if (!key) {
		return Object.keys(treeOriginal);
	}
